(function initChatUIAppScrollFocusWorkflow(root) {
  ...
  function createScrollFocusWorkflow(deps = {}) {
    ...
    function scheduleSessionTailFocusAfterLayout(e={}) {
      with (deps) {
        const t=$("messages");if(!t)return scheduleSessionTailFocus(e);cancelSessionTailFocusAfterLayout();const s=++state.scrollVersion,n=Number.isFinite(e.quietMs)?e.quietMs:160,a=Number.isFinite(e.maxMs)?e.maxMs:2400,i=Number.isFinite(e.stableFrames)?Math.max(1,e.stableFrames):3,q=Date.now(),x=Number.isFinite(e.minWaitMs)?Math.max(0,e.minWaitMs):0;let o=null,r=null,l=null,d=null,c=null,m=!1,h=new WeakSet,k=null;const A=()=>Math.max(0,t.scrollHeight-t.scrollTop-t.clientHeight),E=()=>{e.pinDuringLayout&&(k=A())},P=()=>{if(!e.pinDuringLayout||null===k)return;const s=Math.max(0,t.scrollHeight-t.clientHeight-k);setMessagesProgrammaticScroll();t.scrollTop=s;state.lastMessageScrollTop=t.scrollTop;updateResumeStreamButton()},F=()=>{P();requestAnimationFrame(()=>{state.scrollVersion===s&&!m&&P()})};if(!1!==e.immediate){focusSessionTail(e),E(),requestAnimationFrame(()=>{state.scrollVersion===s&&!m&&(focusSessionTail(e),E())}),setTimeout(()=>{state.scrollVersion===s&&!m&&(focusSessionTail(e),E())},80)};const u=()=>{const e=document.scrollingElement||document.documentElement;return [t.scrollHeight,t.clientHeight,t.children.length,e?.scrollHeight||0,document.body?.scrollHeight||0,innerHeight||0].join(":")},p=()=>{clearTimeout(o),o=null,clearTimeout(c),c=null,r?.disconnect?.(),l?.disconnect?.(),d&&cancelAnimationFrame(d),d=null,m=!0,sessionTailFocusCleanup=null},g=()=>{if(state.scrollVersion!==s||m)return;p();if(e.releaseBooting){try{document.body.classList.remove("app-booting")}catch{}}try{e.onDone?.()}catch{}};let f=()=>{if(state.scrollVersion!==s||m)return;setMessagesProgrammaticScroll();focusSessionTail(e);E();requestAnimationFrame(()=>{state.scrollVersion===s&&!m&&(focusSessionTail(e),E())});setTimeout(()=>{state.scrollVersion===s&&!m&&(focusSessionTail(e),E())},120);setTimeout(g,260)};const w=()=>{if(state.scrollVersion!==s||m)return;let e="",t=0;const n=()=>{if(state.scrollVersion!==s||m)return;const snapshot=u();snapshot===e?t+=1:(e=snapshot,t=1);if(t>=i){const e=q+x-Date.now();if(e>0)return o=setTimeout(w,e);return f()}d=requestAnimationFrame(n)};d=requestAnimationFrame(n)},y=()=>{clearTimeout(o),o=setTimeout(w,n)},S=()=>{if(!r)return;[t,...t.querySelectorAll(".message,.bubble-wrap,.bubble,.content,.reasoning-panel,.markdown-body,img,video,iframe,table,pre,.code-block,.mermaid,.mermaid-block,.markdown-mermaid-pending,svg,canvas")].forEach(e=>{try{h.has(e)||(h.add(e),r.observe(e))}catch{}})};if("ResizeObserver"in window){r=new ResizeObserver(()=>{if(state.scrollVersion!==s)return;F(),y()}),S()}if("MutationObserver"in window){l=new MutationObserver(()=>{if(state.scrollVersion!==s)return;S(),F(),y()}),l.observe(t,{childList:!0,subtree:!0,attributes:!0,attributeFilter:["class","style","src","data-persisted-src","width","height","viewBox","data-mermaid-rendered","data-state"]})}const b=!!e.finalOnly;if(b){f=()=>{if(state.scrollVersion!==s||m)return;setMessagesProgrammaticScroll();focusSessionTail(e);E();const a=Number.isFinite(e.postFocusQuietMs)?e.postFocusQuietMs:360;clearTimeout(o),o=setTimeout(()=>{if(state.scrollVersion!==s||m)return;const hasPendingImages=[...t.querySelectorAll("img")].some(e=>!e.complete);if(hasPendingImages)return y();focusSessionTail(e),E(),g()},a)}}const v=[...t.querySelectorAll("img")].filter(e=>!e.complete);v.forEach(e=>{try{e.addEventListener("load",()=>{F(),y()},{once:!0}),e.addEventListener("error",()=>{F(),y()},{once:!0})}catch{}});document.fonts?.ready?.then?.(()=>{state.scrollVersion===s&&(F(),y())}).catch?.(()=>{});requestAnimationFrame(()=>requestAnimationFrame(y));c=setTimeout(()=>{state.scrollVersion===s&&!m&&f()},b?a:Math.min(a,520));sessionTailFocusCleanup=()=>{clearTimeout(o),clearTimeout(c),r?.disconnect?.(),l?.disconnect?.(),d&&cancelAnimationFrame(d),sessionTailFocusCleanup=null,m=!0};return s
      }
    }
    ...
    function settleActiveOutput(e,t={}) {
      with (deps) {
        if(!e?.isConnected||state.userScrollLocked)return;const s=$("messages"),n=Number.isFinite(t.durationMs)?Math.max(160,t.durationMs):900,a=Date.now()+n,i=()=>{if(!e?.isConnected||state.userScrollLocked)return!1;if(e.dataset.sessionId&&e.dataset.sessionId!==state.activeSessionId)return!1;pinNodeBottomToTarget(e,t);return!0};i();requestAnimationFrame(()=>i());[50,150,320,700].forEach(e=>setTimeout(()=>i(),e));if(!s)return;let o=null,r=null,l=!1;const d=()=>{if(l)return;if(Date.now()>a||!i()){c();return}clearTimeout(o),o=setTimeout(d,90)},c=()=>{l=!0;clearTimeout(o);try{r?.disconnect?.()}catch{}};if("ResizeObserver"in window){try{r=new ResizeObserver(d);[e,...e.querySelectorAll(".content,.reasoning-panel,.markdown-body,img,video,iframe,table,pre,.code-block,.mermaid,.mermaid-block")].forEach(e=>r.observe(e))}catch{}}setTimeout(c,n+80)
      }
    }
